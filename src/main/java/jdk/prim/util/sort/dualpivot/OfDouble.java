...
/**
 * A {@code double} implementation of {@link java.util.DualPivotQuicksort}
 */
public final class OfDouble {

    private OfDouble() {}

    public static void sort(double[] a, PrimitiveComparator.OfDouble comparator, int parallelism, int low, int high) {
    ...
    private static void sort(Sorter sorter, double[] a, PrimitiveComparator.OfDouble comparator, int bits, int low, int high) {
        while (true) {
             ...
             *    1 ------------o-----o------------
             */
            if (comparator.compareDouble(a[e5], a[e2]) < 0) { double t = a[e5]; a[e5] = a[e2]; a[e2] = t; }
            if (comparator.compareDouble(a[e4], a[e1]) < 0) { double t = a[e4]; a[e4] = a[e1]; a[e1] = t; }
            if (comparator.compareDouble(a[e5], a[e4]) < 0) { double t = a[e5]; a[e5] = a[e4]; a[e4] = t; }
            if (comparator.compareDouble(a[e2], a[e1]) < 0) { double t = a[e2]; a[e2] = a[e1]; a[e1] = t; }
            if (comparator.compareDouble(a[e4], a[e2]) < 0) { double t = a[e4]; a[e4] = a[e2]; a[e2] = t; }

            if (comparator.compareDouble(a3, a[e2]) < 0) {
                if (comparator.compareDouble(a3, a[e1]) < 0) {
                    a[e3] = a[e2]; a[e2] = a[e1]; a[e1] = a3;
                } else {
                    a[e3] = a[e2]; a[e2] = a3;
                }
            } else if (comparator.compareDouble(a3, a[e4]) > 0) {
                if (comparator.compareDouble(a3, a[e5]) > 0) {
             ...
             */
            if (comparator.compareDouble(a[e1], a[e2]) < 0 && comparator.compareDouble(a[e2], a[e3]) < 0 && comparator.compareDouble(a[e3], a[e4]) < 0 && comparator.compareDouble(a[e4], a[e5]) < 0) {
                ...
                */
                int[] pivotIndices = partition(a, low, high, e1, e5, comparator, OfDouble::partitionDualPivot);
                 ...
                 */
                int[] pivotIndices = partition(a, low, high, e3, e3, comparator, OfDouble::partitionSinglePivot);
                lower = pivotIndices[0];
                upper = pivotIndices[1];
    ...
    private static int[] partitionDualPivot(double[] a, int low, int high, int pivotIndex1, int pivotIndex2, PrimitiveComparator.OfDouble comparator) {
        ...
        */
        while (comparator.compareDouble(a[++lower], pivot1) < 0);
        while (comparator.compareDouble(a[--upper], pivot2) > 0);
        ...
        for (int unused = --lower, k = ++upper; --k > lower; ) {
            ...
            if (comparator.compareDouble(ak, pivot1) < 0) { // Move a[k] to the left side
                while (lower < k) {
                    if (comparator.compareDouble(a[++lower], pivot1) >= 0) {
                        if (comparator.compareDouble(a[lower], pivot2) > 0) {
                            a[k] = a[--upper];
                            a[upper] = a[lower];
                        } else {
                            a[k] = a[lower];
            ...
            } else if (comparator.compareDouble(ak, pivot2) > 0) { // Move a[k] to the right side
                a[k] = a[--upper];
                a[upper] = ak;
            }
        }
    ...
    private static int[] partitionSinglePivot(double[] a, int low, int high, int pivotIndex1, int pivotIndex2, PrimitiveComparator.OfDouble comparator) {
        ...
        */
        ...
        for (int k = ++upper; --k > lower; ) {
            ...
            if (comparator.compareDouble(ak, pivot) != 0) {
                a[k] = pivot;

                if (comparator.compareDouble(ak, pivot) < 0) { // Move a[k] to the left side
                    while (comparator.compareDouble(a[++lower], pivot) < 0);

                    if (comparator.compareDouble(a[lower], pivot) > 0) {
                        a[--upper] = a[lower];
                    }
                    a[lower] = ak;
    ...
    private static void mixedInsertionSort(double[] a, int low, int high, PrimitiveComparator.OfDouble comparator) {
        ...
        if (end == high) {
            ...
            for (int i; low < high; ++low) {
                 ...
                 */
                if (comparator.compareDouble(a1, a2) > 0) {

                    while (comparator.compareDouble(a1, a[--i]) < 0) {
                        a[i + 2] = a[i];
                    }
                    a[++i + 1] = a1;

                    while (comparator.compareDouble(a2, a[--i]) < 0) {
                        a[i + 1] = a[i];
                    }
                    a[i + 1] = a2;

                } else if (comparator.compareDouble(a1, a[i - 1]) < 0) {
    ...
    private static void insertionSort(double[] a, int low, int high, PrimitiveComparator.OfDouble comparator) {
        for (int i, k = low; ++k < high; ) {
            ...
            if (comparator.compareDouble(ai, a[i - 1]) < 0) {
                while (--i >= low && comparator.compareDouble(ai, a[i]) < 0) {
                    a[i + 1] = a[i];
                }
                a[i + 1] = ai;
            }
     ...
     * @param high the index of the last element, exclusive, to be sorted
     */
    private static void heapSort(double[] a, int low, int high, PrimitiveComparator.OfDouble comparator) {
     ...
     * @param high the index of the last element, exclusive, to be sorted
     */
    private static void pushDown(double[] a, int p, double value, int low, int high, PrimitiveComparator.OfDouble comparator) {
        for (int k ;; a[p] = a[p = k]) {
            ...
            if (k == high || comparator.compareDouble(a[k], a[k - 1]) < 0) {
                --k;
            }
            if (comparator.compareDouble(a[k], value) <= 0) {
                break;
            }
        }
        a[p] = value;
     ...
     */
    private static boolean tryMergeRuns(Sorter sorter, double[] a, PrimitiveComparator.OfDouble comparator, int low, int size) {
        ...
        for (int k = low + 1; k < high; ) {
             ...
             */
            if (comparator.compareDouble(a[k - 1], a[k]) < 0) {

                // Identify ascending sequence
                while (++k < high && comparator.compareDouble(a[k - 1], a[k]) <= 0);

            } else if (comparator.compareDouble(a[k - 1], a[k]) > 0) {

                // Identify descending sequence
                while (++k < high && comparator.compareDouble(a[k - 1], a[k]) >= 0);

                // Reverse into ascending order
                for (int i = last - 1, j = k; ++i < --j && comparator.compareDouble(a[i], a[j]) > 0; ) {
                    double ai = a[i]; a[i] = a[j]; a[j] = ai;
                }
            } else { // Identify constant sequence
                for (double ak = a[k]; ++k < high && comparator.compareDouble(ak, a[k]) == 0; );
            ...
            } else if (comparator.compareDouble(a[last - 1], a[last]) > 0) {
     ...
     */
    private static double[] mergeRuns(double[] a, double[] b, PrimitiveComparator.OfDouble comparator, int offset,
    ...
    private static void mergeParts(Merger merger, double[] dst, int k,
            double[] a1, int lo1, int hi1, double[] a2, int lo2, int hi2, PrimitiveComparator.OfDouble comparator) {
        ...
        if (merger != null && a1 == a2) {
            ...
            while (true) {
                ...
                for (int loo = lo2; loo < mi2; ) {
                    ...
                    if (comparator.compareDouble(key, a2[t]) > 0) {
                        loo = t + 1;
                    } else {
                        mi2 = t;
                    }
        ...
        while (lo1 < hi1 && lo2 < hi2) {
            dst[k++] = comparator.compareDouble(a1[lo1], a2[lo2]) < 0 ? a1[lo1++] : a2[lo2++];
        }
        if (dst != a1 || k < lo1) {
            while (lo1 < hi1) {
                dst[k++] = a1[lo1++];
    ...
    private static final class Merger extends CountedCompleter<Void> {
        ...
        private final PrimitiveComparator.OfDouble comparator;
    ...
    private static final class RunMerger extends RecursiveTask<double[]> {
        ...
        private RunMerger(double[] a, double[] b, PrimitiveComparator.OfDouble comparator, int offset,
