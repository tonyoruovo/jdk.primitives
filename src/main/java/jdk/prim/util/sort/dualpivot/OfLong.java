...
/**
 * A {@code long} implementation of {@link java.util.DualPivotQuicksort}
 */
public final class OfLong {

    private OfLong() {}

    public static void sort(long[] a, PrimitiveComparator.OfLong comparator, int parallelism, int low, int high) {
    ...
    private static void sort(Sorter sorter, long[] a, PrimitiveComparator.OfLong comparator, int bits, int low, int high) {
        while (true) {
            ...
            if (size < MAX_MIXED_INSERTION_SORT_SIZE + bits && (bits & 1) > 0) {
                Constants.sort(a, low, high, comparator, OfLong::mixedInsertionSort);
                return;
            }
            ...
            if (size < MAX_INSERTION_SORT_SIZE) {
                Constants.sort(a, low, high, comparator, OfLong::insertionSort);
                return;
            }
             ...
             *    1 ------------o-----o------------
             */
            if (comparator.compareLong(a[e5], a[e2]) < 0) { long t = a[e5]; a[e5] = a[e2]; a[e2] = t; }
            if (comparator.compareLong(a[e4], a[e1]) < 0) { long t = a[e4]; a[e4] = a[e1]; a[e1] = t; }
            if (comparator.compareLong(a[e5], a[e4]) < 0) { long t = a[e5]; a[e5] = a[e4]; a[e4] = t; }
            if (comparator.compareLong(a[e2], a[e1]) < 0) { long t = a[e2]; a[e2] = a[e1]; a[e1] = t; }
            if (comparator.compareLong(a[e4], a[e2]) < 0) { long t = a[e4]; a[e4] = a[e2]; a[e2] = t; }

            if (comparator.compareLong(a3, a[e2]) < 0) {
                if (comparator.compareLong(a3, a[e1]) < 0) {
                    a[e3] = a[e2]; a[e2] = a[e1]; a[e1] = a3;
                } else {
                    a[e3] = a[e2]; a[e2] = a3;
                }
            } else if (comparator.compareLong(a3, a[e4]) > 0) {
                if (comparator.compareLong(a3, a[e5]) > 0) {
             ...
             */
            if (comparator.compareLong(a[e1], a[e2]) < 0 && comparator.compareLong(a[e2], a[e3]) < 0 && comparator.compareLong(a[e3], a[e4]) < 0 && comparator.compareLong(a[e4], a[e5]) < 0) {
                ...
                */
                int[] pivotIndices = partition(a, low, high, e1, e5, comparator, OfLong::partitionDualPivot);
                 ...
                 */
                int[] pivotIndices = partition(a, low, high, e3, e3, comparator, OfLong::partitionSinglePivot);
                lower = pivotIndices[0];
                upper = pivotIndices[1];
    ...
    private static int[] partitionDualPivot(long[] a, int low, int high, int pivotIndex1, int pivotIndex2, PrimitiveComparator.OfLong comparator) {
        int end = high - 1;
        int lower = low;
        int upper = end;
        ...
        */
        while (comparator.compareLong(a[++lower], pivot1) < 0);
        while (comparator.compareLong(a[--upper], pivot2) > 0);
        ...
        for (int unused = --lower, k = ++upper; --k > lower; ) {
            ...
            if (comparator.compareLong(ak, pivot1) < 0) { // Move a[k] to the left side
                while (lower < k) {
                    if (comparator.compareLong(a[++lower], pivot1) >= 0) {
                        if (comparator.compareLong(a[lower], pivot2) > 0) {
                            a[k] = a[--upper];
                            a[upper] = a[lower];
                        } else {
                            a[k] = a[lower];
            ...
            } else if (comparator.compareLong(ak, pivot2) > 0) { // Move a[k] to the right side
                a[k] = a[--upper];
                a[upper] = ak;
            }
        }
     ...
     * @param pivotIndex2 the index of pivot2, the second pivot
     */
    private static int[] partitionSinglePivot(long[] a, int low, int high, int pivotIndex1, int pivotIndex2, PrimitiveComparator.OfLong comparator) {
        ...
        */
        ...
        for (int k = ++upper; --k > lower; ) {
            ...
            if (comparator.compareLong(ak, pivot) != 0) {
                a[k] = pivot;

                if (comparator.compareLong(ak, pivot) < 0) { // Move a[k] to the left side
                    while (comparator.compareLong(a[++lower], pivot) < 0);

                    if (comparator.compareLong(a[lower], pivot) > 0) {
                        a[--upper] = a[lower];
                    }
                    a[lower] = ak;
     ...
     */
    private static void mixedInsertionSort(long[] a, int low, int high, PrimitiveComparator.OfLong comparator) {
        int size = high - low;
        ...
        if (end == high) {
            ...
            for (int i; ++low < end; ) {
                ...
                while (comparator.compareLong(ai, a[--i]) < 0) {
                    a[i + 1] = a[i];
                }
                a[i + 1] = ai;
            }
            ...
            for (int i, p = high; ++low < end; ) {
                ...
                if (comparator.compareLong(ai, a[i - 1]) < 0) { // Small element
                ...
                } else if (p > i && comparator.compareLong(ai, pin) > 0) { // Large element

                    /*
                     * Find element smaller than pin.
                     */
                    while (comparator.compareLong(a[--p], pin) > 0);
            ...
            for (int i; low < high; ++low) {
                 ...
                 */
                if (comparator.compareLong(a1, a2) > 0) {

                    while (comparator.compareLong(a1, a[--i]) < 0) {
                        a[i + 2] = a[i];
                    }
                    a[++i + 1] = a1;

                    while (comparator.compareLong(a2, a[--i]) < 0) {
                        a[i + 1] = a[i];
                    }
                    a[i + 1] = a2;

                } else if (comparator.compareLong(a1, a[i - 1]) < 0) {
     ...
     * @param high the index of the last element, exclusive, to be sorted
     */
    private static void insertionSort(long[] a, int low, int high, PrimitiveComparator.OfLong comparator) {
        for (int i, k = low; ++k < high; ) {
            long ai = a[i = k];

            if (comparator.compareLong(ai, a[i - 1]) < 0) {
                while (--i >= low && comparator.compareLong(ai, a[i]) < 0) {
                    a[i + 1] = a[i];
                }
                a[i + 1] = ai;
            }
     ...
     * @param high the index of the last element, exclusive, to be sorted
     */
    private static void heapSort(long[] a, int low, int high, PrimitiveComparator.OfLong comparator) {
     ...
     * @param high the index of the last element, exclusive, to be sorted
     */
    private static void pushDown(long[] a, int p, long value, int low, int high, PrimitiveComparator.OfLong comparator) {
        for (int k ;; a[p] = a[p = k]) {
            ...
            if (k == high || comparator.compareLong(a[k], a[k - 1]) < 0) {
                --k;
            }
            if (comparator.compareLong(a[k], value) <= 0) {
                break;
            }
        }
        a[p] = value;
     ...
     */
    private static boolean tryMergeRuns(Sorter sorter, long[] a, PrimitiveComparator.OfLong comparator, int low, int size) {
        ...
        for (int k = low + 1; k < high; ) {
             ...
             */
            if (comparator.compareLong(a[k - 1], a[k]) < 0) {

                // Identify ascending sequence
                while (++k < high && comparator.compareLong(a[k - 1], a[k]) <= 0);

            } else if (comparator.compareLong(a[k - 1], a[k]) > 0) {

                // Identify descending sequence
                while (++k < high && comparator.compareLong(a[k - 1], a[k]) >= 0);

                // Reverse into ascending order
                for (int i = last - 1, j = k; ++i < --j && comparator.compareLong(a[i], a[j]) > 0; ) {
                    long ai = a[i]; a[i] = a[j]; a[j] = ai;
                }
            } else { // Identify constant sequence
                for (long ak = a[k]; ++k < high && comparator.compareLong(ak, a[k]) == 0; );
            ...
            } else if (comparator.compareLong(a[last - 1], a[last]) > 0) {
     ...
     */
    private static long[] mergeRuns(long[] a, long[] b, PrimitiveComparator.OfLong comparator, int offset,
    ...
    private static void mergeParts(Merger merger, long[] dst, int k,
            long[] a1, int lo1, int hi1, long[] a2, int lo2, int hi2, PrimitiveComparator.OfLong comparator) {
        ...
        if (merger != null && a1 == a2) {
            ...
            while (true) {
                ...
                for (int loo = lo2; loo < mi2; ) {
                    ...
                    if (comparator.compareLong(key, a2[t]) > 0) {
                        loo = t + 1;
                    } else {
                        mi2 = t;
                    }
        ...
        while (lo1 < hi1 && lo2 < hi2) {
            dst[k++] = comparator.compareLong(a1[lo1], a2[lo2]) < 0 ? a1[lo1++] : a2[lo2++];
        }
        if (dst != a1 || k < lo1) {
            while (lo1 < hi1) {
                dst[k++] = a1[lo1++];
    ...
    private static final class Sorter extends CountedCompleter<Void> {
        ...
        private final PrimitiveComparator.OfLong comp;

        private Sorter(CountedCompleter<?> parent,
                long[] a, long[] b, int low, int size, int offset, int depth, PrimitiveComparator.OfLong comp) {
            super(parent);
            this.a = a;
            this.b = b;
            this.low = low;
        ...
        private void forkSorter(int depth, int low, int high) {
            addToPendingCount(1);
            long[] a = this.a; // Use local variable for performance
    ...
    private static final class Merger extends CountedCompleter<Void> {
        private static final long serialVersionUID = 20180818L;
        private final long[] dst, a1, a2;
        private final int k, lo1, hi1, lo2, hi2;
        private final PrimitiveComparator.OfLong comparator;

        private Merger(CountedCompleter<?> parent, long[] dst, int k,
                long[] a1, int lo1, int hi1, long[] a2, int lo2, int hi2, PrimitiveComparator.OfLong comparator) {
            super(parent);
            this.dst = dst;
            this.k = k;
        ...
        private void forkMerger(long[] dst, int k,
     ...
     */
    private static final class RunMerger extends RecursiveTask<long[]> {
        private static final long serialVersionUID = 20180818L;
        private final long[] a, b;
        private final PrimitiveComparator.OfLong comparator;
        ...
        private RunMerger(long[] a, long[] b, PrimitiveComparator.OfLong comparator, int offset,
