 ...
 * {@link #ifPresent(PrimitiveConsumer.OfByte) ifPresent()} (performs an
 * action if a value is present).
 *
 * @apiNote
 * {@code OptionalByte} is primarily intended for use as a method return type where
 * there is a clear need to represent "no result." A variable whose type is
 * {@code OptionalByte} should never itself be {@code null}; it should always point
 * to an {@code OptionalByte} instance.
 */
